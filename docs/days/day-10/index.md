...
- 看 Everyday Types 的 Functions 部分：https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#functions
  - 必看章节：Function Type Expressions、Call Signatures、Construct Signatures、Optional Parameters、Function Overloads 只略读。
  - 读完要会：能写函数参数类型和返回值类型。
- 看 Object Types：https://www.typescriptlang.org/docs/handbook/2/objects.html
  - 必看章节：Property Modifiers、Extending Types、Intersection Types、Generic Object Types。
  - 读完要会：能写可选属性、只读属性、接口继承。
- 看 Interfaces 相关内容：https://www.typescriptlang.org/docs/handbook/2/objects.html
  - 在 Object Types 页面重点看：Interface Declaration、Extending Interfaces、Interfaces vs Intersections。
  - 读完要会：能用 `interface User` 描述用户对象。
